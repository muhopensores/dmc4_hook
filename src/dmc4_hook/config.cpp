...
std::vector<ONLOAD> loadCallbacksVec;
std::vector<ONSAVE> saveCallbacksVec;
...
void loadCallbackRegister(ONLOAD loadCb) {
	loadCallbacksVec.push_back(loadCb);
}

void saveCallbackRegister(ONSAVE saveCb) {
	saveCallbacksVec.push_back(saveCb);
}

static void updateMods() {
	// modNamespace::loadConfig(g_config) goes here
	for (auto &functionPtr : loadCallbacksVec) {
		if (functionPtr) {
			functionPtr(g_config);
		}
	}
...
void updateConfig() {
	...
	for (auto &functionPtr : saveCallbacksVec) {
		if (functionPtr) {
			functionPtr(g_config);
		}
	}
