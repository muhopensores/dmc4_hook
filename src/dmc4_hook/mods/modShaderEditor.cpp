...
std::optional<std::string> ShaderEditor::onInitialize() {
    ...
    {
        HL_LOG_ERR("Failed to init sheder mod\n");
...
//void ModShaderEditor::onConfigSave(utils::Config& cfg) { cfg.set<variable_type>("config_string",variable_name); };

// onConfigLoad
// load data into variables from config structure.
//void ModShaderEditor::onConfigLoad(const utils::Config& cfg) { //variable_name = cfg.get<variable_type>("config_string").value_or(default_value); };
