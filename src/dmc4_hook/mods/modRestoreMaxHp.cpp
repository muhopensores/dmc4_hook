...
static bool limitToGround = false;
...
void RestoreMaxHp::onFrame(fmilliseconds& dt) {
    ...
    if (uLocalPlr) {
        ...
        if (modEnabled) {
            ...
            else if (!limitToGround) {
                reset_hp_and_timer:
                if (desiredInput & 0x10 && desiredInput & 0x08) {
...
void RestoreMaxHp::onGUIframe() {
    ...
    ImGui::Checkbox("Disable Aerial Resets", &limitToGround);
    ImGui::SameLine();
    HelpMarker("Disable \"Restore Enemy HP\" and \"Reset Timer\" while aerial (Useful for Taunt Ecstasy)");
}
...
void RestoreMaxHp::onConfigLoad(const utils::Config& cfg) {
    ...
    limitToGround = cfg.get<bool>("reset_hp_limit_to_ground").value_or(false);
}
...
void RestoreMaxHp::onConfigSave(utils::Config& cfg) {
    ...
    cfg.set<bool>("reset_hp_limit_to_ground", limitToGround);
}
