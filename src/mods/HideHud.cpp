...
std::optional<std::string> HideHud::on_initialize() {
    ...
    utility::create_keyboard_hotkey(HideHud::m_hotkeys, {VK_OEM_MINUS}, __("Hide All HUD"), "hide_all_hud");

    console->system().RegisterCommand("hud", "Toggle all HUD", [this]() {
        mod_enabled_hide_all_hud = !mod_enabled_hide_all_hud;
        this->hide_all_hud(mod_enabled_hide_all_hud);
    });
...
void HideHud::on_update_input(utility::Input& input) {
    if (HideHud::m_hotkeys[0]->check(input)) {
        mod_enabled_hide_all_hud = !mod_enabled_hide_all_hud;
...
void HideHud::on_gui_frame(int display) {
    ...
    if (ImGui::Checkbox(_("Hide Timer"), &mod_enabled_timer)) {
        toggle_timer(mod_enabled_timer);
    }
    ImGui::SameLine();
    ...
    if (ImGui::Checkbox(_("Hide Style HUD"), &mod_enabled_style)) {
        toggle_style(mod_enabled_style);
    }
    ImGui::SameLine(sameLineWidth);
    if (ImGui::Checkbox(_("Style Dial Only"), &mod_enabled_boey)) {
    ...
    ImGui::SameLine(sameLineWidth);
    if (ImGui::Checkbox(_("Hide Boss HP HUD"), &mod_enabled_boss)) {
        toggle_boss_hp(mod_enabled_boss);
    }
    ImGui::SameLine();
