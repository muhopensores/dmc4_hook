...
class RoomRespawn : public Mod {
public:
    RoomRespawn() = default;

    // override getModName() method with your mod name for logging.
    std::string get_mod_name() override { return "RoomRespawn"; };
...
private:
    ...
    std::unique_ptr<FunctionHook> m_script_enemy_set_ctrl_opcode_mission;
    std::unique_ptr<FunctionHook> m_script_enemy_set_ctrl_opcode_bp;
