...
void EnemyTracker::on_frame(fmilliseconds& dt) {
    if (SMediator* sMedPtr = devil4_sdk::get_sMediator()) {
        if (sMedPtr->player_ptr) {
            ...
            "?????????????~......:^^~~^!J?~?YY5YJ??????????????\n"
            "??????????????!~~^^^^JPPBGPBBPP5GBBGY?????????????\n"
            "?????????????J55YJJY5GBGGGGBGP55GBBBBP????????????\n"
            "???????????YPGGBGPGGBBGGPGBGP555GBBBBBP???????????\n"
            "??????????YPGGGGP5PGP?~J?PGGP555PGBBBBB5??????????\n"
            "?????????JPPGBBG5~:~^....:?G55PPPGGGGGBBY?????????\n"
            "?????????PGBBBBP~.:^::::..JPPBBBBGGGGGBBY?????????\n"
            "????????5PGBB#G~..^^::::::^?PGGGBBBBBBGY??????????\n"
            "????????PGGBB#B^.:::::::::^JGGGGBBBBBGJ???????????\n"
            "????????JPBBB##P~..:^:::.:YBBBBGGGGBBP????????????\n"
            "?????????5BBB##GY?~::^:.:5GGBBBGGPPBBBY???????????\n"
            "????????JGGGBBB5?YGPY?!~YGPPBBBGGGGBBBG???????????\n"
            "????????YPGBBBBJ??JGBBBGGGPGBGPGGGPPGBBPJ?????????\n"
            "???????JPPBBBBG???~~?PBBGPPGY77PGGGGPGBBGY????????\n"
            "???????5PGGGBBGJ?~::.:?PBBY^^!!?BGGGPGGGGGP5??????\n"
            "??????YGGGBBBBG7~^^::.:!PJ..~!!~PBGGGGGGGBBGJ?????\n"
            "??????PGGBBBGJ~:::^^^^^~!^.^~^::7BGGGGGGBBBGJ?????\n"
            "??????JPBBBY^..::::::^!7~^^::....5BBBBBBBBBP??????\n"
            "????JJJPBB7.........::^7!^:......^JPGBBBBBB5?JJJJ?\n"
            "???JYPGBB!...........:^!~..........:^7PBGBBBGGGPY?\n"
            "????YPGB!.............:~:.........^J5PGBBBGB#BG5??\n"
            "?????JJ?..............:^:..........:?5YYPPJ7Y5YJ??\n"
            "??????^:....  .......::^:.... .........:~7:.^?????\n"
            ...
            "??????????^............:7?:.............:~????????\n"
            "??????????JJ~:.......:!YBBJ^...........:~?????????\n"
            "???????????PBGPYJ?JY5GBBBBBBPJ7~~^~~!?Y5J?????????\n"
            "???????????JPGGGPGGGGGPPPPPGBBBBGGGBBBB5??????????\n"
            "????????????Y55YYYYY55PPPPP5555555555P5???????????\n"
            "????????????JP5YYYYY55PPGP5555YYYYYY5PY???????????\n"*/
            // char xyzText[64];
            // snprintf(xyzText, sizeof(xyzText), "X: %.2f\nY: %.2f\nZ: %.2f", sMedPtr->player_ptr->m_pos.x, sMedPtr->player_ptr->m_pos.y, sMedPtr->player_ptr->m_pos.z);
            // static Vector3f textPos = { 0.0f, 500.0f, 0.0f };
            // static Vector3f textRot = { 0.0f, 0.0f, 0.0f };
            // static ImColor textCol = { 1.0f, 1.0f, 1.0f, 1.0f };
            // static float textWidth = 0.0f;
            // static float textSize = 50.0f;
            // w2s::Draw3DText(xyzText, textPos, textRot, textCol, textWidth, textSize, 1.0f, 1.0f, 1.0f, 1.0f);
            //ImGui::SliderFloat3("textPos", &textPos.x, -1200.0f, 1200.0f);
            //ImGui::SliderFloat3("textRot", &textRot.x, -180.0f, 180.0f);
            //ImGui::ColorEdit4("textCol", (float*)&textCol);
            //ImGui::SliderFloat("textWidth", &textWidth, 0.0f, 20.0f, "%.0f");
