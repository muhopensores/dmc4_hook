...
void __stdcall wew(MtDTI* dti, char* path) {
    ...
    //case 0x6C6F4372: extension = "col"; break;
...
naked void detour3()
{
    __asm{
        cmp byte ptr [exists], 1
        jne originalcode
        mov byte ptr [exists], 0
        xor eax, eax
    originalcode:
        mov esi, eax
        test esi, esi
        je jmp_je
        jmp dword ptr [jmp_return02]
    jmp_je:
        jmp dword ptr [detour2_conditional]
    }
}
...
void LoadOrder::toggle(bool enable) {
    if (enable) 
    {
        ...
        // install_hook_absolute(0x8DFCF0,m_hook,&detour,&jmp_return,5);
        // install_hook_absolute(0x8DFD62,m_hook1,&detour2,&jmp_return01,4);
        // m_hook1 = std::make_unique<FunctionHook>(0x8DFD62,detour2);
        // jmp_return01 = 0x8DFD62 + 4;
    }
    else 
    {
        // m_hook.reset();
...
std::optional<std::string> LoadOrder::on_initialize() {
    ...
    // if (!install_hook_absolute(0x8DF575, m_hook2, &detour3, &jmp_return02, 6)) {
    //     spdlog::error("Failed to init LoadOrder mod\n");
    //     return "Failed to init LoadOrder mod";
    // }
...
{ 
    if (ImGui::Checkbox("WIP HDD file priority instead of .arc", &mod_enabled))
    {
        toggle(mod_enabled);
    }
};
