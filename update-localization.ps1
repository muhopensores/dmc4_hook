...
New-Item -ItemType Directory -Force -Path .\i18n\po\new
New-Item -Path .\i18n\po\new\ -Name dmc4hook.po

Get-ChildItem -Path .\src\ -Filter *.cpp -Recurse -File | ForEach-Object {
    xgettext --default-domain=dmc4hook --output=.\i18n\po\new\dmc4hook.po --join-existing --from-code=utf-8 --c++ --keyword=_ --width=80 $_.FullName
    $content = Get-Content -Path .\i18n\po\new\dmc4hook.po
    $econtent = $content -replace 'charset=CHARSET', 'charset=UTF-8'
    Set-Content -Path .\i18n\po\new\dmc4hook.po -Value $econtent
}
